...
function renderInvoiceItemRow(index, item) {
    ...
    const effPrice = (Number(item.don_gia) || 0) * (1 - giamGia / 100);
    const lineTotal = (Number(item.so_luong) || 0) * effPrice;
    return `
        <tr>
            <td>
                <input type="text"
...
function renderInvoiceItems() {
    ...
    const total = invoiceItems.reduce(
        (sum, it) => sum + (Number(it.so_luong) || 0) * (Number(it.don_gia) || 0) * (1 - (Number(it.ti_le_giam_gia) || 0) / 100),
        0
    );
...
invoiceItemRows.addEventListener("input", (event) => {
    ...
    const effPrice =
        ...
        (1 - (Number(invoiceItems[idx].ti_le_giam_gia) || 0) / 100);
    const lineTotal = (Number(invoiceItems[idx].so_luong) || 0) * effPrice;
    ...
    try {
        ...
        const payload = {
            ...
            items: invoiceItems.map((it) => ({
                ...
                don_gia: Math.round((Number(it.don_gia) || 0) * (1 - (Number(it.ti_le_giam_gia) || 0) / 100)),
            })),
        };
