...
Project dùng Flask API, PostgreSQL và Redis. Backend hiện chỉ trả JSON cho API client, không còn render HTML template.
...
| PostgreSQL | localhost:5432 | `techstore` / `MyPass@2025` mặc định, DB `quan_ly_chi_nhanh` |
...
Các route dưới đây là roadmap dựa trên schema trong `init/postgres/01-schema-and-data.sql`. Những route có dấu `*` là route đã có.
...
```env
BRANCH_CN01_DB_HOST=
BRANCH_CN01_DB_ENGINE=postgresql
BRANCH_CN01_DB_PORT=5432
BRANCH_CN01_DB_NAME=
BRANCH_CN01_DB_USER=
...
Backend trong folder này chỉ có driver PostgreSQL (`psycopg`). Nếu một chi nhánh dùng SQL Server hoặc MySQL, cần cài thêm driver Python và viết thêm hàm kết nối tương ứng trước khi query dữ liệu thật.
