...
def get_branch_db_settings(branch_code):
    ...
    is_current_branch = (
        (current_app.config.get("APP_ROLE") or "").lower() == "branch"
        and (current_app.config.get("BRANCH_CODE") or "").upper() == branch_code.upper()
    )
    ...
    if is_current_branch:
        settings.update(
            {
                "host": settings["host"] or current_app.config.get("DB_HOST"),
                "port": settings["port"] or current_app.config.get("DB_PORT"),
                "name": settings["name"] or current_app.config.get("DB_NAME"),
                "user": settings["user"] or current_app.config.get("DB_USER"),
                "password": settings["password"]
                or current_app.config.get("DB_PASSWORD"),
            }
        )
...
def get_branch_db_engine(branch_code):
    ...
    if (
        ...
        default_engine = current_app.config.get("DB_ENGINE", "sqlserver")
    engine = os.getenv(f"BRANCH_{normalized_code}_DB_ENGINE", default_engine)
